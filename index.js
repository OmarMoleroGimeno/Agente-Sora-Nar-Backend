...
const authenticateToken = async (req, res, next) => {
  ...
  try {
    ...
    if (dbError || !dbUser) {
      console.warn(
        `User ${user.email} authenticated via Google but not found in public.users.`,
      );
    ...
    console.log(
      "DEBUG: Auth Success, attached user:",
      req.user?.email,
      req.user?.id,
    );
    next();
...
app.post("/api/auth/set-password", async (req, res) => {
  ...
  try {
    ...
    if (!supabaseAdmin) {
      return res
        .status(500)
        .send("Server misconfiguration: Missing Service Role Key");
    }
    ...
    try {
      ...
      if (
        ...
        if (!existingAuthUser)
          throw new Error("User reported existing but not found");
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      if (msgError || threadsError || usersError)
...
app.post("/api/users", authenticateToken, isAdmin, async (req, res) => {
  ...
  if (!email || !username)
    return res.status(400).send("Email and username required");
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Uploading document: ${sanitizedFilename} (Original: ${originalName})`,
      );
...
app.post("/api/threads", authenticateToken, async (req, res) => {
  console.log(
    "DEBUG: POST /threads handler reached. User:",
    req.user?.id,
    "Body:",
    req.body,
  );
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        ...
        try {
          ...
          const { data: machines, error } = await supabaseAdmin.from(
            "machinery",
          ).select(`
                            model,
                            description,
          ...
          if (machines && machines.length > 0) {
            // Format context
            const contextString = machines
              .map((m) => {
                const cat = m.categories?.name || "General";
                const specs = m.specs ? JSON.stringify(m.specs) : "";
                const power = m.power_consumption_watts
                  ? `${m.power_consumption_watts}W`
                  : m.power_consumption_text;
          ...
          messages: [
            {
              role: "system",
              content:
                "Generate a very short, concise title (max 5 words) for this chat based on the user message. Do not use quotes.",
            },
            { role: "user", content },
          ],
          model: process.env.OPENAI_MODEL || "gpt-4o",
